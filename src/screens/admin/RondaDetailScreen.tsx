...
export const RondaDetailScreen: React.FC<RondaDetailScreenProps> = ({ navigation, route }) => {
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      {loading ? (
      ...
      ) : fixturesSinPartido.length === 0 && partidos.length === 0 ? (
        renderEmptyState()
      ) : (
        <ScrollView
        ...
        >
          {/* Fixtures Sin Partido Section */}
          {isAdmin && fixturesSinPartido.length > 0 && (
            <View style={styles.section}>
              <View style={styles.sectionHeader}>
                <MaterialCommunityIcons name="alert-circle" size={24} color={colors.warning} />
                <Text style={styles.sectionTitle}>
                  Fixtures Sin Partido ({fixturesSinPartido.length})
                </Text>
              </View>
              <Text style={styles.sectionSubtitle}>
                Asigna fecha, hora y cancha a cada fixture para crear los partidos
              </Text>

              {fixturesSinPartido.map((fixture, index) => renderFixtureSinPartido(fixture, index))}

              <Button
                title="Crear Partidos"
                onPress={handleCreatePartidos}
                loading={creatingPartidos}
                disabled={creatingPartidos}
